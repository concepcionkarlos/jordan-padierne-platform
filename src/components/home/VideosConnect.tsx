...
export default function VideosConnect({ hasVideos }: { hasVideos: boolean }) {
  const { t } = useT()
  return (
    <>
      {!hasVideos && (
        <Reveal className="max-w-2xl mx-auto bg-white rounded-3xl shadow-card p-10 text-center mb-10">
          ...
          <h3 className="font-serif text-2xl font-bold text-navy-900 mb-2">{t('videos.emptyTitle')}</h3>
          <p className="text-gray-500 mb-6">{t('videos.emptySub')}</p>
        </Reveal>
      )}
      ...
      <Reveal className="flex flex-col sm:flex-row items-center justify-center gap-3">
        <a href={SOCIAL_LINKS.youtube.url} target="_blank" rel="noopener noreferrer"
          ...
          <YouTubeIcon /> {t('videos.subscribe')}
        </a>
