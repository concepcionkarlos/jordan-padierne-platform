...
export default function LeadWorkspace({ lead: initialLead, initialNotes, initialTasks, initialAppointments, messages }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* ═══════════ Coach: Next Best Action — always first (mobile + desktop) ═══════════ */}
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-6 lg:items-start">
        ...
        <aside className="lg:col-span-4 xl:col-span-3 lg:sticky lg:top-6 space-y-4 mb-6 lg:mb-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            <div className="flex items-start gap-3">
              <div className="w-11 h-11 rounded-full bg-navy-50 flex items-center justify-center shrink-0 font-serif font-bold text-navy-700">
                {(lead.full_name || '?').trim().charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <p className="font-serif text-lg font-bold text-navy-900 truncate leading-tight">{lead.full_name}</p>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              ...
              <span className={`inline-flex items-center gap-1 text-xs font-medium ${freshness.className}`}>
          ...
          {/* Temperature (hot-score setter) */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-4">Temperature</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            ...
            <a href={`/portal/${lead.id}`} target="_blank" rel="noopener noreferrer" className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-center gap-1.5 w-full text-xs text-sky-600 hover:text-sky-700 font-semibold">
              ↗ Open client portal
            </a>
          </div>
        ...
        <main className="lg:col-span-8 xl:col-span-9 space-y-6">
          ...
          {/* Outreach — send the buyer qualification form */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            <SectionHeader icon={<Send size={15} className="text-sky-400" />} title="Outreach & forms" />
            <button
              type="button"
